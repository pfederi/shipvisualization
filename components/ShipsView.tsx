...
export default function ShipsView({
  ...
  if (isLoading) {
    return (
      <div className={`p-4 flex flex-col items-center justify-center py-12 flex-1 ${isMobile ? 'pb-32' : 'pb-24'}`}>
  ...
  if (ships.length === 0) {
    return (
      <div className={`p-4 flex-1 ${isMobile ? 'pb-32' : 'pb-24'}`}>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {(dep.internalCourseNumber || dep.officialCourseNumber) && (
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        ...
                        {t.course} {(dep.internalCourseNumber || dep.officialCourseNumber || '').replace(/\b0+(\d+)/g, '$1')}
                      </span>
                    )}
                  </div>
  ...
  // Active ships
  return (
    <div className={`p-4 space-y-4 flex-1 ${isMobile ? 'pb-32' : 'pb-24'}`}>
      {ships.map((ship) => {
            ...
            <div className="mb-3">
              ...
              <h3 className={`font-bold text-lg leading-tight flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-brandblue'}`}>
                ...
                {(ship.internalCourseNumber || ship.courseNumber) && (
                  ...
                  >
                    {t.course} {(ship.internalCourseNumber || ship.courseNumber || '').toString().replace(/\b0+(\d+)/g, '$1')}
                  </span>
                )}
              </div>
