...
export interface LakeConfig {
  ...
  zoom: number
  stations: Station[]
  stationNameMapping: Record<string, string>
  geojsonPath: string
}
...
export const LAKES: Record<string, LakeConfig> = {
  zurichsee: {
    ...
    center: [47.28, 8.6],
    zoom: 12,
    geojsonPath: '/data/export.geojson',
    stations: [
  ...
  greifensee: {
    ...
    stations: [
      { name: 'Maur (See)', latitude: 47.3414, longitude: 8.6714, uic_ref: '8503194' },
      { name: 'Greifensee (See)', latitude: 47.3653, longitude: 8.6758, uic_ref: '8503195' },
      { name: 'Niederuster (See)', latitude: 47.3478, longitude: 8.6925, uic_ref: '8503196' },
      { name: 'Fällanden (See)', latitude: 47.3742, longitude: 8.6589, uic_ref: '8503197' },
      { name: 'Mönchaltorf (See)', latitude: 47.3189, longitude: 8.7056, uic_ref: '8503198' },
    ],
    stationNameMapping: {
      'Maur': 'Maur (See)',
      'Greifensee': 'Greifensee (See)',
      'Niederuster': 'Niederuster (See)',
...
export function normalizeStationName(name: string, lakeId: string = 'zurichsee'): string {
  if (!name) return name
  const config = LAKES[lakeId] || LAKES.zurichsee
  
  // Bekannte Varianten abfangen
  const upperName = name.toUpperCase()
  for (const [key, value] of Object.entries(config.stationNameMapping)) {
...
export function getStationCoordinates(lakeId: string = 'zurichsee'): Map<string, { lat: number; lon: number }> {
  const map = new Map()
  const config = LAKES[lakeId] || LAKES.zurichsee
